...
for yIdx = 1:length(years)
    ...
    CLFI = nan(size(ED));
    CLFI(cropMask) = (ED_nor(cropMask) + PD_nor(cropMask) + (1 - MPA_nor(cropMask))) / 3;

    outFile = fullfile(outputFolder, sprintf('CN_Sugarcane_CLFI_%d.tif', y));
    geotiffwrite(outFile, CLFI, R, 'GeoKeyDirectoryTag',info.GeoTIFFTags.GeoKeyDirectoryTag);
    clear ED PD MPA ED_nor PD_nor MPA_nor CLFI baseMask cropMask outFile
end
